...
for image_input in [224]:
  ...
  for augmentation_type in ['none']:
    ...
    config_path = f"configs/stain/image_size_{image_input}/augmentation_{augmentation_type}/scheduler_type_reduce_on_plateau/criterion_weight_None/optimizer_type_Adam/optimizer_lr_0.0001/loss_function_CrossEntropyLoss/model_{args.model}.yaml"
        ...
        for optimizer_type in ['Adam']:
          for optimizer_lr in [0.0001]:
            for loss_function in ['CrossEntropyLoss', 'TotalCrossEntropyLoss', 'FocalLoss']:
              ...
              config_path = f"configs/stain/{path}.yaml"
              print(f" Processing {config_path}")
              metric_num = 0
              for metric_index in range(5):
                metric_path = f"metrics/stain/{path}/training_dict_fold{metric_index+1}.json"
