...
config_path = f"configs/avg_norm/norm_1/model_{args.model[0]}.yaml"
...
for model in args.model:
    for norm in range(1,21):
        path = f"norm_{norm}/model_{model}"
        config_path = f"configs/avg_norm/{path}.yaml"
        print(f" Processing {config_path}")
        metric_num = 0
        for metric_index in range(5):
          metric_path = f"metrics/avg_norm/{path}/training_dict_fold{metric_index+1}.json"
