...
config_path = f"configs/rmv_inf_grad/rmv_True/model_{args.model[0]}.yaml"
...
for model in args.model:
    for rmv_inf_grad in [True,False]:
        path = f"rmv_{rmv_inf_grad}/model_{model}"
        config_path = f"configs/rmv_inf_grad/{path}.yaml"
        print(f" Processing {config_path}")
        metric_num = 0
        for metric_index in range(5):
          metric_path = f"metrics/rmv_inf_grad/{path}/training_dict_fold{metric_index+1}.json"
