...
BASE_METRICS_DIR = "metrics/bias2_corr"
OUTPUT_DETAILED_PATH = \
    "analysis/tables/bias2_corr_model_evaluation_detailed.csv"
...
def build_metric_path(
    base_dir: str, model: str, scale_multiplier: float,label: str, fold: int
) -> str:
...
def add_metrics(
    ...
    for key in key_list:
        for metric_name, metric_value in dict_metrics[key].items():
            if metric_name in ("Model", "Scale", "Fold"):
                continue
...
def process_metric_data(
    ...
    for key, logits in logits_dict.items():
        logger.info(
            "Processing head '%s' for model %s, scale %.2f, fold %d",
            key,
            model,
            scale_multiplier,
            fold,
    ...
    summary = compute_summary(metrics_list, model, scale_multiplier)
