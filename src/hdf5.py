...
config_path = f"configs/finale2/image_size_1024/model_Resnet18.yaml"
...
def show_sample(dataset, idx=0, title=None, denorm=None):
    """Visualize one (image, labels) pair from a PyTorch Dataset."""
    img, lbl = dataset[idx]

    # Convert to something plt.imshow can handle
    if isinstance(img, torch.Tensor):
        ...
        x = x.numpy()
        # If float image, clip to displayable range
        if x.dtype != np.uint8:
            x = np.clip(x, 0, 1)
    ...
    plt.title(title or f"Labels: {lbl}")
    plt.axis('off')
    plt.show()

# Example: visualize the first item from your existing dataset
show_sample(full_dataset, idx=0)
