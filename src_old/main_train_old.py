...
from utils.hyperparameters import initialize_optimizer, initialize_scheduler, initialize_loss_function
from utils import (construct_directory_structure, save_config_snapshot, set_seed, convert_to_serializable)
from augmentations import get_augmentation_pipeline
from datasets import HDF5Dataset
from models import get_bayesian_model
from trainer import run_training_fold
from evaluation import monte_carlo_prediction
...
def main():
    ...
    # Read HDF5 dataset (check for required datasets)
    try:
        ...
        logging.error(f"HDF5 file not found at path: {config.paths.hdf5_path}")
        raise
    ...
    try:
        ...
        logging.error(f"Error during splitting data: {e}")
        raise

    # Ensure no overlap between splits
    assert len(set(train_val_indices).intersection(test_indices)) == 0, "Training and test sets overlap!"
    logging.info(f"Total samples: {total_samples}")
    logging.info(f"Training and validation samples: {len(train_val_indices)}")
    logging.info(f"Testing samples: {len(test_indices)}")

    # Create test dataset and DataLoader with improved prefetching.
    test_dataset = HDF5Dataset(
        ...
        cache_in_memory=config.HDF5Dataset.cache_in_memory  # Set to True if the dataset fits in RAM
    )
    test_loader = DataLoader(
        ...
        prefetch_factor=config.data.prefetch_factor  # Increased prefetch factor for faster data loading
    ) 
    ...
    if args.resume_checkpoint is not None:
        if not isfile(args.resume_checkpoint):
            logging.error(f"Checkpoint file not found: {args.resume_checkpoint}")
        ...
        try:
            ...
            fold_metrics = checkpoint_data['metrics']
            logging.info(f"Resumed training from fold {checkpoint_data['fold']} and epoch {checkpoint_data['epoch']}")
        except Exception as e:
    ...
    try:
        for fold, (train_idx, val_idx) in enumerate(
            ...
            if args.resume_checkpoint and checkpoint_data:
                if fold == checkpoint_data.get('fold', 0) + 1:
                    try:
                        ...
                        logging.info(f"Loaded states from checkpoint for fold {fold}")
                    except Exception as e:
                        logging.error(f"Failed to load state dict from checkpoint: {e}")
                        raise
            ...
            # Run Monte Carlo prediction on the test set.
            ...
            output_dict_per_fold = {
                ...
                'all_testing_dict': metrics_for_fold['all_testing_dict']
            }
            ...
            try:
                ...
                logging.info(f"Training and evaluation completed. Metrics saved to {metrics_save_path}")
            except Exception as e:
