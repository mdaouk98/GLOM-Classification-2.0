...
config_path = f"configs/multihead_stain/scale_n4.yaml"
...
for scale in ['n4','n3','n2','n1',0,'p1','p2','p3','p4']:
    path = f"scale_{scale}"
    config_path = f"configs/multihead_stain/{path}.yaml"
    print(f" Processing {config_path}")
    metric_num = 0
    for metric_index in range(5):
      metric_path = f"metrics/multihead_stain/{path}/training_dict_fold{metric_index+1}.json"
