# src/multihead_t_iterate_train.py
...
config_path = f"configs/multihead_{multihead_type}/{loss_function}/{model}/scale_0.yaml"
...
for scale in [0,'n_1','n_5','n1','n1_5','n2','n2_5','n5','n10']:
    path = f"scale_{scale}"
    config_path = f"configs/multihead_{multihead_type}/{loss_function}/{model}/{path}.yaml"
    print(f" Processing {config_path}")
    metric_num = 0
    for metric_index in range(5):
      metric_path = f"metrics/multihead_{multihead_type}/{loss_function}/{model}/{path}/training_dict_fold{metric_index+1}.json"
