...
parser.add_argument(
     '--inputsize', '-i',
     nargs='+',                      # <-- one or more values
     default=[224],           # <-- now default is a list
     help='One or more image input size, e.g. --inputsize 224 384'
 )
parser.add_argument('--resume_checkpoint', type=str, default=None,
                    help='Path to resume training from a checkpoint.')
args = parser.parse_args()

for image_input in args.inputsize:
  # Load configuration and set up directories.
    config_path = f"configs/finale2/image_size_{image_input}/model_{args.model[0]}.yaml"
    ...
    for model in args.model:
        path = f"image_size_{image_input}/model_{model}"
        config_path = f"configs/finale2/{path}.yaml"
        print(f" Processing {config_path}")
        metric_num = 0
        for metric_index in range(5):
          metric_path = f"metrics/finale2/{path}/training_dict_fold{metric_index+1}.json"
    ...
    # 1) close the dataset if it has a method for it
    if hasattr(full_dataset, 'close'):
        full_dataset.close()
    # 2) drop the reference
    del full_dataset
    # 3) force-GC so memory is reclaimed immediately
    gc.collect()