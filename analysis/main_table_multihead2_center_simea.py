...
DEFAULT_CONFIG_PATH = \
    "/home/mdaouk/Repo/configs/finale2/image_size_224/model_Resnet18.yaml"
BASE_METRICS_DIR = "metrics/multihead2"
OUTPUT_DETAILED_PATH = \
    "analysis/tables/multihead2_center_model_evaluation_detailed.csv"
...
def build_metric_path(
    base_dir: str, model: str, scale_multiplier: float, cluster: int, fold: int
) -> str:
    """Construct the file path for a specific metric JSON file."""

    return os.path.join(
        base_dir,
        f"scale_{scale_multiplier}",
        f"cluster_{cluster}",
        f"model_{model}",
...
def add_metrics(
    ...
    for key in key_list:
        for metric_name, metric_value in dict_metrics[key].items():
            if metric_name in ("Model", "Scale", "Cluster", "Fold"):
                continue
...
def process_metric_data(
    ...
    summary = compute_summary(metrics_list, model, scale_multiplier, cluster)
        ...
        for cluster in clusters:
            for fold in folds:
                metric_path = build_metric_path(
                    base_dir, model, scale_multiplier, cluster, fold
                )
...
def main(
    ...
    scale_multipliers: Iterable[float] = SCALE_MULTIPLIERS,
    clusters: Iterable[int] = CLUSTERS,
