...
def monte_carlo_prediction(
    ...
    # 1) Put entire model into eval (freeze BatchNorm, etc.)
    model.eval()
    model.to(device)

    # 2) Re-enable ONLY the final-layer Dropout for MC sampling
    #    (assumes your BayesianModel has an attribute `.dropout`)
    ...
    for iteration in range(n_iter):
        ...
        # Sanity check: did we cover exactly all samples this iteration?
        if sample_count != total_samples:
            raise RuntimeError(
                f"After iteration {iteration+1}, expected {total_samples} samples "
                f"but processed {sample_count}"
            )
