...
for image_input in args.inputsize:
  # Load configuration and set up directories.
    config_path = f"configs/bias2/stain/model_{args.model[0]}.yaml"
    ...
    for label in ['stain','scanner']:
        for model in args.model:
            path = f"{label}/model_{model}"
            config_path = f"configs/bias2/{path}.yaml"
            print(f" Processing {config_path}")
            metric_num = 0
            for metric_index in range(5):
                metric_path = f"metrics/bias2/{path}/training_dict_fold{metric_index+1}.json"
