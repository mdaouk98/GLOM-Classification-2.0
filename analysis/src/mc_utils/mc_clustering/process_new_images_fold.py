# src/mc_utils/mc_clustering/process_new_images_fold.py
...
def process_new_images_fold(fold_idx: int,
                            indices: np.ndarray,
    ...
    fold_mc_features: List[np.ndarray] = []
    ...
    if not fold_mc_softmax_probabilities:
        ...
        return None
    fold_mc_features_array = np.stack(fold_mc_features, axis=0)
    fold_mc_softmax_probabilities_array = np.stack(fold_mc_softmax_probabilities, axis=0)
    logging.info(f"Fold {fold_idx}: Collected MC features with shape {fold_mc_features_array.shape}.")
    logging.info(f"Fold {fold_idx}: Collected MC softmax probabilities with shape {fold_mc_softmax_probabilities_array.shape}.")
    return fold_mc_features_array, fold_mc_softmax_probabilities_array
